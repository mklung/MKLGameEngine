...
namespace sge {
	...
	void FileStream::openRead(StrView filename) {
		open(filename, FileMode::OpenExists, FileAccess::ReadOnly, FileShareMode::Read);
	}
...
#pragma mark ========= Windows ============
	...
	void FileStream::open(StrView filename, FileMode mode, FileAccess access, FileShareMode shareMode) {
		...
		switch (access) {
		case FileAccess::ReadOnly:			access_flags |= GENERIC_READ;					break;
	...
	u64 FileStream::fileSize() {
		...
		auto fileSize = static_cast<u64>(high) << 32 | low;
		return fileSize;
	}

	void FileStream::setFileSize(u64 newSize) {
		_ensure_fd();
		u64 oldPos = getPos();
		setPos(newSize);
		::SetEndOfFile(_fd);
	...
	u64 FileStream::getPos() {
		_ensure_fd();
		LONG high = 0;
		LONG low = ::SetFilePointer(_fd, 0, &high, FILE_CURRENT);
		if (low < 0 || high < 0) throw SGE_ERROR("get file pos");
		auto pos = static_cast<u64>(low) | static_cast<u64>(high) << 32;
		return pos;
	}

	void FileStream::setPos(u64 pos) {
		_ensure_fd();
	...
	void FileStream::setPosFromEnd(u64 pos) {
		_ensure_fd();
		LONG high = static_cast<LONG>(pos >> 32);
		LONG low = static_cast<LONG>(pos);
