...
namespace sge
{
	...
	protected:
		struct ConstBuffer
		{
			...
			using Info		= ConstBufferInfo;
			using VarInfo	= ShaderVariable;
		...
		private:
			...
			template<class V>
			void _setValueAs(const VarInfo* varInfo, const V& v)
			{
				auto end = varInfo->offset + sizeof(value);
				if (end > cpuBuffer.size())
					throw SGE_ERROR("ConstBuffer setParam out of rangr");
	...
	protected:
		...
		void _setParam(StrView name, const V& v)
		{
			if(_vertexStage) _vertexStage->_ser
		}
